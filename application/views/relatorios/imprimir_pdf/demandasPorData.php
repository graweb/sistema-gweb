	...
	<div>
		...
		<table class="table">
			...
			<tbody>
				<?php 
					$i = 0;
					foreach ($demandasMes as $demMes) {
						$dataHora = date('d/m/Y', strtotime($demMes->data_hora));
						switch ($demMes->situacao) {
						    case 1:
						        $situacao = "ABERTO";
						        break;
						    case 2:
						...
						if($demMes->cod_incidente == null) {
							echo '<td style="text-align:center"> R - ' . $demMes->cod_requisicao . '</td>';
						} else {
							echo '<td style="text-align:center">I - ' . $demMes->cod_incidente . '</td>';
						}

						echo '<td>' . $demMes->usuario . '</td>';
						echo '<td>' . $demMes->assunto . '</td>';
						echo '<td>' . $situacao . '</td>';
