...
<body>
	<div>
		...
		<h1 style="text-align:center">Relatório de Demandas por Usuário</h1></td>
		...
		<table class="table">
			...
			<tbody>
				<?php 
					$i = 0;
					foreach ($demandasUsuario as $demUsuario) {
						$dataHora = date('d/m/Y', strtotime($demUsuario->data_hora));
						switch ($demUsuario->situacao) {
						    case 1:
						        $situacao = "ABERTO";
						        break;
						    case 2:
						...
						if($demUsuario->cod_incidente == null) {
							echo '<td style="text-align:center"> R - ' . $demUsuario->cod_requisicao . '</td>';
						} else {
							echo '<td style="text-align:center">I - ' . $demUsuario->cod_incidente . '</td>';
						}

						echo '<td>' . $demUsuario->usuario . '</td>';
						echo '<td>' . $demUsuario->assunto . '</td>';
						echo '<td>' . $situacao . '</td>';
