...
<body>
	<div>
		...
		<h1 style="text-align:center">Relatório de Nível de Atendimento</h1></td>
		...
		<table class="table">
			<thead>
				<tr>
					...
					<th style="width:90px">OBSEVAÇÕES</th>
			...
			<tbody>
				<?php 
					$i = 0;
					foreach ($gerarPdfNivelAtendimento as $relNvlAtend) {
						$dataHora = date('d/m/Y', strtotime($relNvlAtend->data_cadastro));
						switch ($relNvlAtend->situacao) {
						    case 1:
						        $situacao = "ATIVO";
						        break;
						    case 0:
						...
						echo '<tr>';
						echo '<td>' . $relNvlAtend->id_nivel_atendimento . '</td>';
						echo '<td>' . $relNvlAtend->nivel . '</td>';
						echo '<td>' . $relNvlAtend->observacoes . '</td>';
						echo '<td>' . $situacao . '</td>';
